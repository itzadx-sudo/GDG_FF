import sqlite3
import datetime

# Import your vault for encryption
try:
    from vault import encrypt_email, decrypt_email, hash_password, verify_password
except ImportError:
    # Fallback if running directly
    from vault import encrypt_email, decrypt_email, hash_password, verify_password

DB_NAME = "orbit.db"

def get_db_connection():
    conn = sqlite3.connect(DB_NAME)
    conn.row_factory = sqlite3.Row 
    return conn

def init_db():
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS candidates (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            email TEXT NOT NULL, 
            password_hash TEXT NOT NULL,
            filename TEXT,
            secure_path TEXT,
            role TEXT DEFAULT 'PILOT',
            match_score INTEGER DEFAULT 0,
            skills_detected TEXT,
            weakness_focus TEXT,
            mission_status TEXT DEFAULT 'ANALYZING',
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        )
    ''')
    
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS job_applications (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            candidate_id INTEGER NOT NULL,
            job_id TEXT NOT NULL,
            job_title TEXT NOT NULL,
            status TEXT DEFAULT 'PENDING',
            applied_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            FOREIGN KEY (candidate_id) REFERENCES candidates (id)
        )
    ''')
    
    conn.commit()
    conn.close()

# ==========================================
#  USER REGISTRATION
# ==========================================
def add_candidate(email, password, filename, secure_path, role='PILOT'):
    conn = get_db_connection()
    cursor = conn.cursor()
    
    safe_email = encrypt_email(email)       # AES-256
    safe_pass = hash_password(password)     # SHA-256 Hash
    
    cursor.execute('''
        INSERT INTO candidates (email, password_hash, filename, secure_path, role, mission_status)
        VALUES (?, ?, ?, ?, ?, 'AWAITING_UPLOAD')
    ''', (safe_email, safe_pass, filename, secure_path, role))
    
    new_id = cursor.lastrowid
    conn.commit()
    conn.close()
    return new_id

# ==========================================
#  USER LOGIN
# ==========================================
def login_user(input_email, input_password):
    conn = get_db_connection()
    rows = conn.execute('SELECT * FROM candidates').fetchall()
    conn.close()
    
    found_user = None
    
    for row in rows:
        try:
            db_email = decrypt_email(row['email'])
            if db_email == input_email:
                if verify_password(row['password_hash'], input_password):
                    found_user = dict(row)
                    found_user['email'] = db_email 
                    break
        except:
            continue
            
    return found_user

# ==========================================
#  FETCHING DATA
# ==========================================
def get_pilot_status(candidate_id):
    conn = get_db_connection()
    row = conn.execute('SELECT * FROM candidates WHERE id = ?', (candidate_id,)).fetchone()
    conn.close()
    
    if row:
        data = dict(row)
        data['email'] = decrypt_email(data['email'])
        return data
    return None

def fetch_all_pilots():
    conn = get_db_connection()
    rows = conn.execute("SELECT * FROM candidates WHERE role = 'PILOT'").fetchall()
    conn.close()
    
    results = []
    for row in rows:
        try:
            data = dict(row)
            data['email'] = decrypt_email(data['email'])
            results.append(data)
        except:
            continue
    return results

def update_ai_results(candidate_id, score, weakness, status):
    conn = get_db_connection()
    conn.execute('''
        UPDATE candidates 
        SET match_score = ?, weakness_focus = ?, mission_status = ?
        WHERE id = ?
    ''', (score, weakness, status, candidate_id))
    conn.commit()
    conn.close()

# ==========================================
#  JOB APPLICATIONS
# ==========================================
def apply_to_job(candidate_id, job_id, job_title):
    """Submit a job application"""
    conn = get_db_connection()
    cursor = conn.cursor()
    
    # Check if already applied
    existing = conn.execute(
        'SELECT * FROM job_applications WHERE candidate_id = ? AND job_id = ?',
        (candidate_id, job_id)
    ).fetchone()
    
    if existing:
        conn.close()
        return {'success': False, 'message': 'Already applied to this position'}
    
    cursor.execute('''
        INSERT INTO job_applications (candidate_id, job_id, job_title, status)
        VALUES (?, ?, ?, 'PENDING')
    ''', (candidate_id, job_id, job_title))
    
    app_id = cursor.lastrowid
    conn.commit()
    conn.close()
    return {'success': True, 'message': 'Application submitted', 'app_id': app_id}

def get_user_applications(candidate_id):
    """Get all applications for a specific user"""
    conn = get_db_connection()
    rows = conn.execute('''
        SELECT * FROM job_applications 
        WHERE candidate_id = ? 
        ORDER BY applied_at DESC
    ''', (candidate_id,)).fetchall()
    conn.close()
    
    return [dict(row) for row in rows]

def get_all_applications():
    """Get all job applications (for admin view)"""
    conn = get_db_connection()
    rows = conn.execute('''
        SELECT ja.*, c.email, c.secure_path, c.filename, c.match_score, c.skills_detected
        FROM job_applications ja
        JOIN candidates c ON ja.candidate_id = c.id
        ORDER BY ja.applied_at DESC
    ''').fetchall()
    conn.close()
    
    results = []
    for row in rows:
        data = dict(row)
        try:
            data['email'] = decrypt_email(data['email'])
        except:
            pass
        results.append(data)
    return results

def update_application_status(app_id, new_status):
    """Update the status of a job application"""
    conn = get_db_connection()
    conn.execute('''
        UPDATE job_applications 
        SET status = ?, updated_at = CURRENT_TIMESTAMP
        WHERE id = ?
    ''', (new_status, app_id))
    conn.commit()
    conn.close()
    return {'success': True, 'message': 'Status updated'}

def get_job_recommendations(candidate_id):
    """Get job recommendations based on user skills (simple matching for now)"""
    user_data = get_pilot_status(candidate_id)
    if not user_data:
        return []
    
    # For now, return jobs that match user's skill level
    # In production, this would use ML/AI matching
    score = user_data.get('match_score', 0)
    
    # Mock recommendations based on score
    recommendations = []
    if score >= 80:
        recommendations = ['job_03', 'job_02', 'job_01']  # High-tier roles
    elif score >= 60:
        recommendations = ['job_02', 'job_01']  # Mid-tier roles
    else:
        recommendations = ['job_01']  # Entry roles
    
    return recommendations

if __name__ == "__main__":
    init_db()